...
#
# This script:
# 1. Assumes the target program has already been compiled to code.bin
...
if [ ! -f "$BIN_OUTPUT" ]; then
    echo "Error: Binary not found: $BIN_OUTPUT"
    echo "Compile your program first (e.g. make compile-c-baremetal file=...)"
    exit 1
fi
