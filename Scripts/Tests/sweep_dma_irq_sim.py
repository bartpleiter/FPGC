...
ASM = REPO / os.environ.get("IRQ_SWEEP_ASM",
                            "Tests/host/dma_irq_sim/spi2mem_irq_stress.asm")
...
def main():
    ...
    if USE_RAM:
        ...
        if r.returncode != 0:
            print("asmpy boot failed:", r.stderr); sys.exit(1)
        ram_tmp = REPO / "tmp/test_ram.list"
        r = subprocess.run(f"asmpy {ASM} {ram_tmp}",
                           shell=True, cwd=str(REPO),
                           capture_output=True, text=True)
        if r.returncode != 0:
            print("asmpy ram failed:", r.stderr); sys.exit(1)
        r = subprocess.run(f"python3 {CONV} {ram_tmp} {RAM_LIST}",
                           shell=True, cwd=str(REPO),
                           capture_output=True, text=True)
        if r.returncode != 0:
            print("convert failed:", r.stderr); sys.exit(1)
    else:
        r = subprocess.run(f"asmpy {ASM} {ROM_LIST}", shell=True, cwd=str(REPO),
                           capture_output=True, text=True)
        if r.returncode != 0:
            print("asmpy failed:", r.stderr)
            sys.exit(1)
    r = subprocess.run(f"iverilog -o {OUT} {TB}", shell=True, cwd=str(REPO),
                       capture_output=True, text=True)
    if r.returncode != 0:
        print("iverilog failed:", r.stderr)
        sys.exit(1)

    periods = [50, 80, 110, 150, 200, 300, 500, 800, 1100,
               1500, 1700, 2000, 2500, 3000, 3500, 4000]
    firsts  = [50, 70, 90, 110, 130, 150, 170, 190, 210, 230,
    ...
    with ProcessPoolExecutor(max_workers=workers) as ex:
        ...
        for fut in as_completed(futures):
            ...
            print(f"[{done:4d}/{len(grid)}] PERIOD={p:5d} FIRST={f:4d} -> {tag}",
                  flush=True)
