; Test: VRAM8 clear loop (bootloader pattern)
; Exactly mimics the bootloader ClearVRAM8 loop pattern
; Uses `write 0 r1 r0` which writes r0 (always 0) to address r1+0
; Then reads back and verifies the data is 0

Main:
    ; First, write non-zero values to VRAM8 to ensure it's not already 0
    ...
    WriteNonZeroLoop:
        ...
        beq r3 r2 8          ; if done, skip jump
        jump WriteNonZeroLoop

    ; Now clear VRAM8 exactly like bootloader does
    load32 0x1E800000 r1     ; VRAM8 address
    ...
    load32 0x1E800000 r1     ; VRAM8 base address
