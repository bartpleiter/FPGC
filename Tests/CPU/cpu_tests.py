...
class CPUTestConfig:
    ...
    ASSEMBLER_DIRECTORY = "BuildTools/ASM"
    ASSEMBLER_SCRIPT = "Assembler.py"
...
class CPUTestRunner:
    ...
    def _assemble_code(self, source_path: str, output_path: str, description: str) -> None:
        """
        ...
        copy_cmd = f"cp {source_path} {self.config.TARGET_ASM_PATH}"
        exit_code, output = self._run_command(copy_cmd, f"Copying {description}")
        
        if exit_code != 0:
            raise AssemblerError(f"Failed to copy {description}: {output}")
        
        # Run assembler
        assemble_cmd = f"cd {self.config.ASSEMBLER_DIRECTORY} && python3 {self.config.ASSEMBLER_SCRIPT} -H > code.list"
        exit_code, output = self._run_command(assemble_cmd, f"Assembling {description}")
        
        if exit_code != 0:
            # Try to read error details from code.list
            try:
                with open(f"{self.config.ASSEMBLER_DIRECTORY}/code.list", "r") as error_file:
                    error_details = error_file.read()
                raise AssemblerError(f"Assembler failed for {description}: {error_details}")
            except FileNotFoundError:
                raise AssemblerError(f"Assembler failed for {description}: {output}")
        
        # Move assembled code to target location
        move_cmd = f"mv {self.config.ASSEMBLER_DIRECTORY}/code.list {output_path}"
        exit_code, output = self._run_command(move_cmd, f"Moving assembled {description}")
        
        if exit_code != 0:
            raise AssemblerError(f"Failed to move assembled {description}: {output}")
    ...
    def _assemble_code_to_ram(self, path: str) -> None:
        ...
        self._assemble_code(
            self.config.BOOTLOADER_ROM_PATH, 
            self.config.ROM_LIST_PATH, 
            "ROM bootloader"
        )
        ...
        exit_code, output = self._run_command(convert_cmd, "Converting to 256-bit format")
        ...
        for line in test_lines:
            if "expected=" in line:
                try:
                    ...
                    raise CPUTestError(f"Invalid expected value format in line: {line}") from e
    ...
    def get_test_files(self) -> list[str]:
        ...
        try:
            ...
            raise CPUTestError(f"Tests directory not found: {self.config.TESTS_DIRECTORY}")
        except Exception as e:
