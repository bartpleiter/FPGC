...
BDOS is the operating system, loaded from SPI Flash. It provides a shell, filesystem access (BRFS), hardware drivers, and syscalls for user programs. User programs are loaded from the filesystem into dynamically allocated memory regions and relocated at load time via a relocation table. See [OS](../Software/OS.md).

### Toolchain

- **ASMPY**: Python-based assembler for the B32P3 ISA. See [Assembler](../Software/Assembler.md).
- **cproc + QBE**: C11 compiler toolchain with custom B32P3 backends. Self-hosting on BDOS. See [C Compiler](../Software/C-compiler.md).
