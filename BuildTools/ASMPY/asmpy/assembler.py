...
class Assembler:
    ...
    RELOC_DATA_WORD = 0  # .int label — full 32-bit word
    RELOC_LOAD_PAIR = 1  # addr2reg → load+loadhi pair
    RELOC_JUMP = 2  # jump label — 27-bit address field
    ...
    def __init__(
        ...
        # Relocation table entries: list of (byte_offset, reloc_type)
        self._relocation_entries: list[tuple[int, int]] = []
    ...
    def _remove_directive_lines(self) -> None:
        ...
        self._assembly_lines = [
            ...
            for line in self._assembly_lines
            if not isinstance(
                line, (DirectiveAssemblyLine, AlignDirectiveLine, GlobalDirectiveLine)
            )
        ]
    ...
    def _flush_byte_buffer(
        ...
        for i in range(0, len(byte_buffer), 4):
            ...
            line = DataAssemblyLine(
                ...
                source_line_number=template_line.source_line_number
                if template_line
                else 0,
                source_file_name=template_line.source_file_name
                if template_line
                else "",
            )
    ...
    def _apply_label_address_mappings(self) -> None:
        ...
        When self.independent is True, relocatable entries are tracked for the
        relocation table (data words, load/loadhi pairs, header jumps).
        """
        ...
        for idx, line in enumerate(self._assembly_lines):
            if isinstance(line, DataAssemblyLine) and line.label_ref is not None:
                ...
                if base not in self._label_address_mappings:
                    raise ValueError(f"Undefined label in data: {line.label_ref.label}")
                ...
                # Track for relocation: data word with absolute address
                if self.independent:
                    byte_offset = idx * 4
                    self._relocation_entries.append((byte_offset, self.RELOC_DATA_WORD))
                ...
                for arg in line.arguments:
                    if isinstance(arg, Label):
                        ...
                        if is_branch:
                            ...
                            if self.independent:
                                byte_offset = idx * 4
                                if line.instruction_type == JumpOperation.JUMP:
                                    self._relocation_entries.append(
                                        (byte_offset, self.RELOC_JUMP)
                                    )
                                elif (
                                    line.instruction_type
                                    == SingleCycleArithmeticOperation.LOAD_LOW
                                ):
                                    # load+loadhi pair: record at the load instruction
                                    self._relocation_entries.append(
                                        (byte_offset, self.RELOC_LOAD_PAIR)
                                    )
                                # LOAD_HIGH is handled as part of the LOAD_LOW pair, skip it
    ...
    def _append_relocation_table(self) -> None:
        """Append the relocation table after the program data.

        The table consists of a count word followed by packed relocation entries.
        Each entry is encoded as: [31:8] byte_offset | [7:0] type.
        This is only called when self.independent is True and there are entries.
        """
        if not self._relocation_entries:
            return

        # Sort entries by byte offset for deterministic output
        self._relocation_entries.sort(key=lambda e: (e[0], e[1]))

        # Relocation count word
        count_line = DataAssemblyLine(
            code_str=f".dw {len(self._relocation_entries)}",
            comment="reloc_count",
            original="",
            source_line_number=0,
            source_file_name="reloc_table",
        )
        self._assembly_lines.append(count_line)

        # Relocation entries
        for byte_offset, reloc_type in self._relocation_entries:
            packed = (byte_offset << 8) | (reloc_type & 0xFF)
            entry_line = DataAssemblyLine(
                code_str=f".dw {packed}",
                comment=f"reloc type={reloc_type} off={byte_offset}",
                original="",
                source_line_number=0,
                source_file_name="reloc_table",
            )
            self._assembly_lines.append(entry_line)

        self._logger.info(
            f"Appended relocation table: {len(self._relocation_entries)} entries"
        )
