...
void bdos_shell_start_format_wizard(void)
{
  ...
  term2_puts("BRFS v2 format wizard\n");
  term2_puts("Enter total blocks (multiple of 64):\n");
}
...
int bdos_shell_handle_special_mode_line(char *line)
{
  ...
  if (bdos_shell_mode == BDOS_SHELL_MODE_BOOT_FORMAT_YN)
  {
    ...
    {
      term2_puts("Please answer yes or no.\n");
      return 1;
    }
  ...
  if (bdos_shell_mode == BDOS_SHELL_MODE_FORMAT_BLOCKS)
  {
    ...
    if (value <= 0)
    {
      term2_puts("Invalid block count. Please enter a positive integer.\n");
      return 1;
    }

    bdos_shell_format_blocks = (unsigned int)value;
    bdos_shell_mode = BDOS_SHELL_MODE_FORMAT_WORDS;
    term2_puts("Enter bytes per block (multiple of 256):\n");
    return 1;
  }
  ...
  if (bdos_shell_mode == BDOS_SHELL_MODE_FORMAT_WORDS)
  {
    ...
    if (value <= 0)
    {
      term2_puts("Invalid bytes-per-block. Please enter a positive integer.\n");
      return 1;
    }

    if ((value & 3) != 0)
    {
      term2_puts("Bytes per block must be a multiple of 4.\n");
      return 1;
    }

    bdos_shell_format_words = (unsigned int)value / 4u;
    bdos_shell_mode = BDOS_SHELL_MODE_FORMAT_LABEL;
    term2_puts("Enter label (max 10 chars):\n");
    return 1;
  }
  ...
  if (bdos_shell_mode == BDOS_SHELL_MODE_FORMAT_LABEL)
  {
    if (strlen(line) == 0)
    {
      term2_puts("Label cannot be empty.\n");
      return 1;
    }
...
void bdos_shell_on_startup(void)
{
  ...
  term2_puts("BRFS mount failed: ");
  term2_puts(bdos_fs_error_string(bdos_fs_last_mount_error));
  term2_putchar('\n');

  if (bdos_fs_boot_needs_format)
  {
    term2_puts("Filesystem is required. Format now? (yes/no)\n");
