...
int ov7670_init(void)
{
    ...
    err |= ov_write(0x11, 0x01);  /* CLKRC: prescaler /2 */
    err |= ov_write(0x0C, 0x00);  /* COM3: no DCW/scaling */
    err |= ov_write(0x3E, 0x00);  /* COM14: no manual PCLK scaling */
    err |= ov_write(0x72, 0x11);  /* Scaling XSC (default) */
    err |= ov_write(0x73, 0xF1);  /* Scaling YSC (default) */
    err |= ov_write(0x70, 0x3A);  /* Scaling control */
    err |= ov_write(0x71, 0x35);  /* Scaling control */
    ...
    err |= ov_write(0x13, 0xE5);  /* COM8: re-enable AGC/AEC */
    ...
    /* Night mode */
    //err |= ov_write(0x3B, 0x0A);  /* COM11: night mode enable */
