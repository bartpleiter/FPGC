 ...
 *
 * V2: Register values adapted from a working FPGA reference project
 * (FPGA_OV7670_Camera_Interface), modified for YUV QVGA output.
 */
...
int ov7670_init(void)
{
    ...
    err |= ov_write(0x6B, 0x4A);  /* DBLV: PLL ×4 (V1 proven value) */
    delay_10ms();
    err |= ov_write(0x11, 0x00);  /* CLKRC: prescaler off (V1 proven value) */
    ...
    /* ---- Timing / window ---- */
    err |= ov_write(0x17, 0x14);  /* HSTART (ref value) */
    err |= ov_write(0x18, 0x02);  /* HSTOP (ref value) */
    ...
    err |= ov_write(0x40, 0xC0);  /* COM15: full output range (YUV, not RGB565) */
    err |= ov_write(0x3A, 0x00);  /* TSLB: default byte order (test) */
    err |= ov_write(0x3D, 0xC0);  /* COM13: gamma enable, UV sat auto */
    err |= ov_write(0x15, 0x00);  /* COM10: VSYNC positive, HREF positive */

    /* ---- YUV color matrix (from Linux kernel) ---- */
    err |= ov_write(0x4F, 0x80);  /* MTX1 */
    err |= ov_write(0x50, 0x80);  /* MTX2 */
    err |= ov_write(0x51, 0x00);  /* MTX3 */
    err |= ov_write(0x52, 0x22);  /* MTX4 */
    err |= ov_write(0x53, 0x5E);  /* MTX5 */
    err |= ov_write(0x54, 0x80);  /* MTX6 */
    err |= ov_write(0x58, 0x9E);  /* MTXS */
    ...
    err |= ov_write(0x14, 0x38);  /* COM9: 16× gain ceiling (V1 value) */
    ...
    /* ---- Misc ---- */
    err |= ov_write(0x0F, 0x41);  /* COM6: reset timings (ref value) */
    ...
    err |= ov_write(0x74, 0x00);  /* REG74: no digital gain (ref value) */
    err |= ov_write(0xB0, 0x84);  /* RSVD */
    err |= ov_write(0xB1, 0x0C);  /* ABLC1 */
    err |= ov_write(0xB2, 0x0E);  /* RSVD */
    err |= ov_write(0xB3, 0x80);  /* THL_ST (ref value) */
